...
for i_movie=1:length(data.name)
    ...
    frames_to_check=find(abs(data.NNopen_nnba{i_movie})>120);%these are the frames that are likely wrong
    movie_name=data.name{i_movie};
    movie_name_long=strcat(movie_name,'_for_NNopen_NNAnglesInserted.tif');
    
    num_montages=round_down(length(frames_to_check)/32)+1;%this is basically rounding up
    i_NNopen_frame=0;
    ...
    for n_montage=1:num_montages
        ...
        while i_montage<=images_in_montage
            i_NNopen_frame=i_NNopen_frame+1;
            ...
            if button==31%if I pressed down key, record this frame as wrong
                ...
                accept_NNopen(i_NNopen_frame)=0;
            ...
            elseif button==30%if I pressed up key, record this frame as good
                ...
                accept_NNopen(i_NNopen_frame)=1;
                i_montage=i_montage+1;
            elseif button==28%if I pressed left arrow, go back (effectively one frame).
                i_NNopen_frame=i_NNopen_frame-2;
                i_montage=i_montage-1;
            elseif button==57%if I pressed "9", then the next 8 frames are wrong
                accept_NNopen(i_NNopen_frame:i_NNopen_frame+7)=0;
                i_NNopen_frame=i_NNopen_frame+7;
                i_montage=i_montage+8;
            elseif button==56%if I pressed "8", then the next 8 frames are correct
                accept_NNopen(i_NNopen_frame:i_NNopen_frame+7)=1;
                i_NNopen_frame=i_NNopen_frame+7;
                i_montage=i_montage+8;
            elseif button==48%if I pressed "0", then all remaining of the 32 frames are wrong.
                accept_NNopen(i_NNopen_frame:i_NNopen_frame+(images_in_montage-i_montage))=0;
                i_NNopen_frame=i_NNopen_frame+(images_in_montage-i_montage);
            ...
            elseif button==55%if I pressed "7", then all remaining of the 32 frames are correct.
                accept_NNopen(i_NNopen_frame:i_NNopen_frame+(images_in_montage-i_montage))=1;
                i_NNopen_frame=i_NNopen_frame+(images_in_montage-i_montage);
                i_montage=images_in_montage+1;%break for while loop
            end
        end
    ...
    %delete "NNopen_nnba" results that are wrong
    data.NNopen_nnba{i_movie}(frames_to_check(accept_NNopen==0))=nan;
    close all
end

%now, modify nnba accordingly
for i_movie=1:length(data.name)
    data.nnba{i_movie}(data.NNopen_frames{i_movie})=data.NNopen_nnba{i_movie};
end
