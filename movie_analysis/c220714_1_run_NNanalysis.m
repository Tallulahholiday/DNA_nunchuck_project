...
cd /Users/ambercai/Desktop/matlab/

load('AlexNet_test4_28.mat');
net=AlexNet_test4_28;
...
for i=1:numStacks%for each movie
    ...
    %if a frame is blank, change its "raw_nnba" into nan
    for frame=1:length(stack_ds.Files)%for each frame of this movie
        image=imread(strcat(movie_name,'.TIFF'),'Index',frame);
        brightness_sum(frame)=sum(sum(image));
    end
    raw_nnba(brightness_sum==0)=nan;
    
    %filter out images with low (2 std away) scores, after filtering out blank frames
    clear predsFiltered scoresFiltered brightness_sum
...
save(strcat('/Users/ambercai/Desktop/matlab/data_files/37PPnsIHF_2023_good.mat'),'data');
...
for k=1:numStacks %will split up every stack 
    ...
    for i = 1:numFrames %saves each modified frame as individual file
        ...
        img = cat(3,A,A,A); %changes from 8bit to truecolor
        fileName=strcat(folderPath,name,'.tif'); %filename is frame number
        imwrite(img,fileName); %writes frame
    end
end
num=numStacks;
out=folders;
